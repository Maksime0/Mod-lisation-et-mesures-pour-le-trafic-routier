...
listeDesParametres = reconstruireParametres("results/essaisVitesseLineaire/config/config.txt")
...
temps = 0
rho = reconstruireListe("results/essaisVitesseLineaire/calculs/0.txt")
...
for i in range(1, nbPointsTemps):
    temps = temps + deltaT
    rho = reconstruireListe("results/essaisVitesseLineaire/calculs/" + str(i) + ".txt")
    line.set_ydata(rho)
    plt.pause(1e-5)
